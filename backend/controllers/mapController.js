  ...
  try {
    ...
    const withUrls = await Promise.all(
      maps.map(async (m) => ({
        ...
        thumbnailUrl: await signGetUrl(m.thumbnailKey),
        size: m.size,
...
export const getMap = async (req, res) => {
  try {
    ...
    res.status(200).json({
      ...
      thumbnailUrl: await signGetUrl(map.thumbnailKey),
      json,
      size: map.size,
...
export const createMap = async (req, res) => {
  try {
    ...
    const newMap = await Map.create({
      ...
      jsonKey,
      thumbnailKey,
      jsonUrl: `https://${BUCKET_NAME}.s3.${process.env.AWS_REGION}.amazonaws.com/${jsonKey}`,
    ...
    res.status(201).json({
      ...
      thumbnailUrl: await signGetUrl(newMap.thumbnailKey),
      size: newMap.size,
...
export const duplicateMap = async (req, res) => {
  try {
    ...
    // Pull the JSON from S3 so we can re-upload under a new key.
    const json = await fetchJsonFromS3(original.jsonKey);
    if (!json) {
    ...
    // Copy the thumbnail too if the original had one (cheap server-side copy).
    let newThumbKey = "";
    if (original.thumbnailKey) {
      newThumbKey = thumbnailKeyFor(userId, newId.toString());
      // Easiest portable approach: download and re-upload. (CopyObjectCommand also works.)
    ...
    const dup = await Map.create({
      ...
      jsonUrl: `https://${BUCKET_NAME}.s3.${process.env.AWS_REGION}.amazonaws.com/${newJsonKey}`,
      thumbnailUrl: newThumbKey
    ...
    res.status(201).json({
      ...
      thumbnailUrl: await signGetUrl(dup.thumbnailKey),
      size: dup.size,

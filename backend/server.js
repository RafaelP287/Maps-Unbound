 ...
 * Handles REST API routes and WebSocket connections for campaign lobbies and encounters.
 *
 * CORE SYSTEMS:
 * 1. REST API: authentication, campaigns, characters, join requests
 * 2. Socket.io: real-time lobby chat, encounter state sync, turn management
 * 3. Encounter Engine:
 *    - Readiness gate: DM gates player access via isReady flag
 *    - Ownership enforcement: players move only controlled tokens
 *    - Turn economy: resource tracking (actions, bonus actions, reactions, movement)
 *    - 5ft per square: movement cost deduction on move events
 *    - Initiative system: DM manages turn order and round advancement
 *
 * AUTHORIZATION MODEL:
 * - Campaign membership required for all encounter access
 * - DM role checks for prep/readiness operations
 * - Token ownership checks for individual token updates
...
io.on('connection', (socket) => {
  ...
  const resetEncounterTokens = (encounter) => {
    ...
    // Reset returns everyone to a fresh round, restores HP, and rerolls initiative.
    encounter.round = 1;
  ...
  // Only combat-relevant narration goes back to lobby chat; bookkeeping stays in the encounter log.
  ...
  const canActOnTurn = (campaign, encounter, userId) => {
    ...
    // DM can control enemy turns; players can only control their own active token.
  ...
  socket.on('encounter:add-default-enemies', async ({ campaignId, userId }) => {
    try {
      ...
      // Seed a predictable enemy line-up before combat starts so the DM can build initiative order quickly.

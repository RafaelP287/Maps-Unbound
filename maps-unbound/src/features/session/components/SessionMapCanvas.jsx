...
function SessionMapCanvas({
    ...
    useEffect(() => {
        ...
        import("socket.io-client").then(({ io }) => {
            ...
            socket.on("connect", () => {
                socket.emit("joinSession", { sessionId }, (resp) => {
                    if (!resp?.ok) {
                        console.warn("[session map] joinSession failed:", resp?.error);
                        return;
                    }
                    fetchLatestSnapshot();
                });
            ...
            console.warn("[session map] socket.io-client load failed:", err.message);
        });
    ...
    return (
        <main className={["session-dm__map", isCombatState ? "is-combat-state" : ""].filter(Boolean).join(" ")}>
            ...
            {!readOnly && <div className="session-dm__map-controls" aria-label="Map quick tools">
                <button
                    type="button"

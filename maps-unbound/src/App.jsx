...
function App() {
    ...
    const initializeDice = useCallback(async (canvasElement) => {
        ...
        try {
            // Create room first, then boot renderer — avoids GPU spinning during network wait
            const roomRes = await fetch("https://dddice.com/api/1.0/room", {
                method: "POST",
                headers: { Authorization: `Bearer ${DDDICE_API_KEY}`, "Content-Type": "application/json" },
                body: JSON.stringify({ name: "Maps Unbound Session", is_public: true }),
            });
            if (!roomRes.ok) throw new Error(`Room creation failed (${roomRes.status})`);

            const slug = (await roomRes.json()).data.slug;
            roomSlugRef.current = slug;

            await startRenderer(canvasElement);
            await dddiceRef.current.connect(slug);
            setIsDiceReady(true);
            console.log("[dddice] Ready! Room:", slug);
        } catch (err) {
            console.error("[dddice] Init failed:", err);
